...
tab_overview, tab_regime, tab_sectors, tab_global, tab_fund, tab_detail, tab_ai = st.tabs(
    ["🧭 Market Overview", "🧠 Market Regime", "🔄 Sector Momentum",
     "🌍 Global & Regional", "💰 Fundamentals", "🔎 Detail", "🤖 AI Commentary"]
)
...
# --------------------------------------------------------------------------- #
# AI Market Commentary (LLM)
# --------------------------------------------------------------------------- #
with tab_ai:
    st.subheader("🤖 AI market commentary")
    st.caption(
        "An LLM turns the day's sentiment, ML regime, and sector momentum into a "
        "plain-English summary. **Descriptive only** — the prompt forbids price "
        "predictions and buy/sell recommendations."
    )

    a1, a2, a3 = st.columns([1, 1, 2])
    provider_label = a1.radio("Provider", ["Claude", "OpenAI"], index=0)
    if provider_label == "Claude":
        model = a2.selectbox("Model", ["claude-opus-4-8", "claude-sonnet-5", "claude-haiku-4-5"], index=0)
        key_name, provider = "ANTHROPIC_API_KEY", "claude"
    else:
        model = a2.text_input("Model", value="gpt-4o-mini")
        key_name, provider = "OPENAI_API_KEY", "openai"
    key_input = a3.text_input(
        key_name, type="password",
        help=f"Optional — leave blank to use {key_name} from the environment or .streamlit/secrets.toml.",
    ).strip()
    ...
    try:
        ...
        secret_key = None
    resolved_key = key_input or secret_key or os.environ.get(key_name)

    # Assemble the analytics context the model will summarize.
    mt_ai = momentum_table(price_data, tickers_in("US Sector"))
    ctx = {
        "as_of": last_session.date().isoformat() if last_session is not None else "the latest close",
        "sentiment_score": sent.get("score"),
        "sentiment_label": sent.get("label"),
        "vix": sent.get("vix"),
        "breadth_50": sent.get("breadth_50"),
        "spy_trend": sent.get("spy_trend"),
        "regime": reg.get("current") if reg else None,
        "regime_days": reg.get("days_in_regime") if reg else None,
        "top_sectors": [(r["name"], float(r["ann_momentum"]), r["classification"])
                        for _, r in mt_ai.head(3).iterrows()] if not mt_ai.empty else [],
        "bottom_sectors": [(r["name"], float(r["ann_momentum"]), r["classification"])
                           for _, r in mt_ai.tail(3).iloc[::-1].iterrows()] if not mt_ai.empty else [],
    }

    with st.expander("Data sent to the model", expanded=False):
        st.json(ctx)

    if st.button("✨ Generate commentary", type="primary"):
        if not resolved_key:
            st.warning(
                f"No API key found. Enter one above, or set **{key_name}** as an "
                "environment variable or in `.streamlit/secrets.toml`."
            )
        else:
            try:
                with st.spinner(f"Asking {provider_label}…"):
                    st.session_state["ai_commentary"] = generate_commentary(
                        ctx, provider=provider, model=model, api_key=resolved_key)
                    st.session_state["ai_commentary_meta"] = f"{provider_label} · {model}"
            except Exception as e:  # surface any provider error to the user
                msg = str(e)
                if any(t in msg.lower() for t in ("auth", "api key", "401")):
                    st.error("Authentication failed — check the API key is valid for this provider.")
                else:
    ...
    if st.session_state.get("ai_commentary"):
        ...
        st.caption(
            f"Generated by {st.session_state.get('ai_commentary_meta', 'an LLM')} from the "
            "metrics above. Descriptive analysis, not investment advice."
        )
